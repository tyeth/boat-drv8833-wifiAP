...
try:
    ...
    poolSocket.bind((HOST, PORT))
    poolSocket.listen(BACKLOG)
    print("Listening")
    inbuf = bytearray(MAXBUF)
    counter=0
    while True:
        print("Accepting connections")
        sslSocket=poolSocket
        sslSocket=ssl_context.wrap_socket(poolSocket, server_side=True)
        ...
        if True: #use_stream:
            ...
            # will succeed.
            try:
                req = clientSslSocket.read(poolSocket,4096)
                print(req)
                if req:
                    poolSocket.write(CONTENT % counter)
